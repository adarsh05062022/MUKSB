"""
MUKSB_cls_scale.py — Step-size (scale) ablation for class-concept erasure.

EXACT same KS-bargaining class-unlearning code as MUKSB_cls.py. The ONLY
difference is that the step-size magnitude (`effective_scale`) is selectable
via --scale_variant, so the four arms of the scale ablation can be run from a
single file while sharing the identical KS bisector direction (g_star):

    harmonic    — 2||gr||||gf|| / (||gr||+||gf||)   (== MUKSB full; already run)
    arithmetic  — (||gr|| + ||gf||) / 2             (Arithmetic Mean)
    min         — min(||gr||, ||gf||)               (Min Norm)
    fixed       — 1.0                               (Fixed α=1, direction-only)

Everything else (direction, mask, pseudo-prompt, optimizer, schedule) is
identical to MUKSB_cls.py, so any difference in the results is attributable
solely to the step-size choice.

The harmonic variant reproduces the existing MUKSB results bit-for-bit.

Usage (run from MUKSB/SD/)
-----
  python MUKSB_cls_scale.py --class_to_forget 0 --scale_variant arithmetic --device 0
"""
import argparse
import gc
import os
...
# ── scale-variant registry ───────────────────────────────────────────────────
SCALE_VARIANT_CHOICES = ("harmonic", "arithmetic", "min", "fixed")


def _effective_scale(norm_gr: torch.Tensor, norm_gf: torch.Tensor, scale_variant: str):
    """Step-size magnitude as a function of the two gradient norms.

    Direction is unchanged across variants; only this scalar multiplier differs.
    """
    if scale_variant == "harmonic":
        # harmonic mean: 2·a·b/(a+b) — conservative, dominated by smaller norm
        return 2.0 * norm_gr * norm_gf / (norm_gr + norm_gf)
    if scale_variant == "arithmetic":
        # arithmetic mean: (a+b)/2
        return 0.5 * (norm_gr + norm_gf)
    if scale_variant == "min":
        # minimum norm: min(a, b)
        return torch.minimum(norm_gr, norm_gf)
    if scale_variant == "fixed":
        # fixed scalar 1.0 — direction-only, no adaptive scaling
        return torch.tensor(1.0, device=norm_gr.device, dtype=norm_gr.dtype)
    raise ValueError(
        f"unknown scale_variant={scale_variant!r}; "
        f"choose from {SCALE_VARIANT_CHOICES}"
    )


# ─────────────────────────────────────────────────────────────────────────────
# KS bargaining core — improved (scale is parametrised)
# ─────────────────────────────────────────────────────────────────────────────

def ks_step(
    gr_flat: torch.Tensor,
    gf_flat: torch.Tensor,
    scale_variant: str = "harmonic",
    eps: float = 1e-8,
):
    ...
    # ── SCALE: selectable step-size magnitude (the ablation knob) ─────────────
    effective_scale = _effective_scale(norm_gr, norm_gf, scale_variant)
...
def MUKSB(
    ...
    alpha,
    logger,
    scale_variant="harmonic",
):
    total_start = time.time()
    logger.info("======== MUKSB (KS Bargaining — Scale Ablation) TRAINING STARTED ========")
    logger.info(f"class_to_forget={class_to_forget}  train_method={train_method}")
    logger.info(f"scale_variant={scale_variant}")
    ...
    if mask_variant is not None:
        ...
        name = (
            f"compvis-cls_{class_to_forget}-MUKSB_scale_{scale_variant}-{mask_variant}"
        ...
        mask = None
        name = (
            f"compvis-cls_{class_to_forget}-MUKSB_scale_{scale_variant}"
            f"-method_{train_method}-lr_{lr}_E{epochs}_U{num_forget}_{EXTRA}"
        )
        ...
        with tqdm(total=len(forget_dl)) as pbar:
            for forget_images, forget_labels in forget_dl:
                ...
                lambda_ks, cos_phi, g_star, effective_scale = ks_step(
                    gr_input, gf_input, scale_variant=scale_variant,
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "MUKSB scale ablation: KS-bargaining class erasure with a "
            "selectable step-size magnitude (Stable Diffusion)"
        )
    )

    # ── scale ablation knob ──────────────────────────────────────────────────
    parser.add_argument("--scale_variant", type=str, default="harmonic",
                        choices=list(SCALE_VARIANT_CHOICES),
                        help=(
                            "Step-size magnitude (direction is identical across all):\n"
                            "  harmonic   — 2ab/(a+b)  (== MUKSB full)\n"
                            "  arithmetic — (a+b)/2    (Arithmetic Mean)\n"
                            "  min        — min(a, b)  (Min Norm)\n"
                            "  fixed      — 1.0        (Fixed alpha=1)"
                        ))
    ...
    logger, log_file = setup_logger(
        name=f"MUKSB_cls{args.class_to_forget}_scale_{args.scale_variant}_{EXTRA}"
    )
    logger.info("======== MUKSB SCALE ABLATION STARTED ========")
