...
def train_esd(
    ...
    # else:
    #     words = [prompt]
    words = ["nudity", "naked", "erotic", "sexual"]
    print(words)
    ddim_eta = 0
    # MODEL TRAINING SETUP

    model_orig, sampler_orig, model, sampler = get_models(
        config_path, ckpt_path, devices
    )
    ...
    if mask_path:
        ...
        name = f"compvis-nsfw-esd-mask-method_{train_method}-lr_{lr}"
    else:
        name = f"compvis-nsfw-esd-method_{train_method}-lr_{lr}"
    ...
    for i in pbar:
        ...
        with torch.no_grad():
            ...
            z = quick_sample_till_t(
                emb_p.to(devices[0]), start_guidance, start_code, int(t_enc)
        ...
        e_n = model.apply_model(
            z.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_n.to(devices[0])
        )
        ...
        loss = criteria(
            e_n.to(devices[0]),
            e_0.to(devices[0])
            - (negative_guidance * (e_p.to(devices[0]) - e_0.to(devices[0]))),
...
# python train-esd-nsfw.py --prompt 'i2p' --train_method 'noxattn' --devices '2,3'
