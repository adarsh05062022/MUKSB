...
SD3/MUNBa_nsfw.py
=================
MUNBa (Nash Bargaining) NSFW concept unlearning for Stable Diffusion 3
(HuggingFace Diffusers, flow-matching DiT).

This is the Nash-bargaining counterpart to SD3/MUKSB_nsfw.py and is kept
*structurally identical* — same SD3 components, dataset, flow-matching
forget/retain losses, importance-mask support, logging and checkpointing —
so the two methods can be compared head-to-head on SD3.  The ONLY difference
is the gradient-merge core: the retain/forget task gradients are combined with
the closed-form **Nash bargaining** weights instead of the magnitude-aware
Kalai–Smorodinsky solution.

SD3 specifics (shared with the MUKSB version):
  ...
  - Forget loss compares velocity predictions
  - Retain loss target = noise - latents (velocity)

Checkpoints are saved under ``models/<run_tag>/`` relative to the working
directory — run this from /scratch/s25017/MUKSB/SD3 so they land next to the
MUKSB checkpoints for comparison.

Usage
-----
conda run -n munba3_sd3 python3 MUNBa_nsfw.py \
...
# ─────────────────────────────────────────────────────────────────────────────
# Nash bargaining core  (closed-form, two-player retain/forget game)
# ─────────────────────────────────────────────────────────────────────────────

def _dot(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    # torch.dot is limited to INT_MAX elements; the SD3 transformer flattens to
    # ~2B params, so an element-wise reduction is required.
    return (a * b).sum()
...
def nash_step(gr_flat: torch.Tensor, gf_flat: torch.Tensor, eps: float = 1e-8):
    ...
    non-negative bargaining weights (α_r, α_f) so that the merged gradient
    ...
    back to a fixed retain + ½·forget mix — mirroring SD/MUNBa_nsfw.py.
    ...
    """
    g1 = _dot(gr_flat, gr_flat)            # ‖g_r‖²
    g2 = _dot(gr_flat, gf_flat)            # g_r · g_f
    g3 = _dot(gf_flat, gf_flat)            # ‖g_f‖²
...
def save_history(losses, name, output_dir="models"):
    ...
    plt.title("MUNBa SD3 training loss")
...
def MUNBa(
    ...
    logger.info("======== MUNBa SD3 NSFW (Nash Bargaining) TRAINING STARTED ========")
    ...
    if mask_variant is not None:
        ...
        run_tag = (
            f"sd3-nsfw-MUNBa-{mask_variant}"
            f"-rho{int(mask_density*100)}pct"
            f"-method_{train_method}-lr_{lr}_E{epochs}_U{num_forget}"
        )
    else:
        mask    = None
        run_tag = f"sd3-nsfw-MUNBa-method_{train_method}-lr_{lr}_E{epochs}_U{num_forget}"
    ...
    logger.info("======== MUNBa SD3 NSFW TRAINING FINISHED ========")
    logger.info(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="MUNBa: Nash-Bargaining NSFW unlearning for Stable Diffusion 3"
    )
    ...
    logger, log_file = setup_logger(name="MUNBa_sd3_nsfw")
    logger.info("======== MUNBa SD3 NSFW STARTED ========")
