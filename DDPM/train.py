...
def parse_args_and_config():
    ...
    parser.add_argument(
        ...
        default=0.5,
        help=(
            "MUKSB retain priority weight in [0, 1]. "
            "0.5 = symmetric KS (default). "
            ">0.5 tilts the KS bisector toward the retain gradient direction "
            "(recommended for 1-of-10 forget setups, e.g. 0.7)."
        ),
    )

    args = parser.parse_args()

    if args.mode in ("saliency_unlearn", "muksb_unlearn"):
    ...
    log_path = os.path.join(config.log_dir, "stdout.txt")
    ...
    handler2 = FlushFileHandler(log_path)
    ...
    print(f">>> Monitor with: tail -f {os.path.abspath(log_path)}\n")
...
def main():
    ...
    try:
        ...
        elif args.mode == "muksb_unlearn":
            runner = Diffusion(args, config)
            runner.muksb_unlearn()
