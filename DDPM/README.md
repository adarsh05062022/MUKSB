...
# Forgetting Training with MUKSB (Kalai-Smorodinsky Bargaining)

MUKSB replaces the additive `forget_loss + alpha * remain_loss` objective
with a **Kalai-Smorodinsky bargaining merge** of the retain and forget
gradients. The merged direction is a `gamma`-weighted bisector of the
unit gradients, scaled by the harmonic mean of their norms. When the
two gradients are anti-parallel the step is skipped (no Pareto-improving
update exists).

Hyper-parameter:
* `--gamma` — retain priority in `[0, 1]`.
  * `0.5` → symmetric KS (default).
  * `>0.5` → bisector tilts toward the retain direction (recommended for
    1-of-10 forget setups, e.g. `0.7`).
  * `<0.5` → bisector tilts toward the forget direction.

Run MUKSB **without** a saliency mask:

```
CUDA_VISIBLE_DEVICES="0,1" python train.py \
    --config cifar10_saliency_unlearn.yml \
...
Run MUKSB **with** a SalUn-style saliency mask (KS bargaining is then
restricted to the masked subspace; unmasked coordinates receive a zero
update):
...
Forget objectives mirror saliency_unlearn:
* `--method rl` — pseudo-class noise matching (default).
* `--method ga` — gradient ascent on the forget batch.

Logs report `lambda_KS`, `cos_phi` (gradient-conflict cosine), the
harmonic step scale, and the running anti-parallel skip rate. A high
skip rate is a sign of severe retain/forget conflict — increase
`--gamma` or use a denser saliency mask.
