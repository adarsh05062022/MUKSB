...
def synflow_importance_score(
    ...
    if not use_data:
        ...
        input, target = input.to(next(model.parameters()).device), target.to(next(model.parameters()).device)
        output = model(input)
...
def global_prune_model(
    model, ratio, method, dataloader=None,
    structured=False, sample_per_classes=25,
    img_size=32, use_data=False, criterion=None, iteration_number=100
):
    if method == "mp":
    ...
    if method == "synflow":
        ...
        for k in range(iteration_number):
            # each_ratio = 1.0 - (1.0 - ratio)**((k + 1) / iteration_number)
            print(f"Pruning {each_ratio} of the model")
            score_dict = synflow_importance_score(model, dataloader, img_size, use_data, criterion)
            if structured:
                pass
            else:
                prune.global_unstructured(
