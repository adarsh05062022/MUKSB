...
def RL(data_loaders, model, criterion, optimizer, epoch, args, mask=None):
    ...
    if args.dataset == "cifar10" or args.dataset == "cifar100" or args.dataset == "TinyImagenet":
        ...
        for it, (image, target) in enumerate(train_loader):
            ...
            optimizer.step()
            # print(f'update time: {time.time() - st:.4f}')
    ...
    elif args.dataset == "svhn" or args.dataset == "celeba":
        losses = utils.AverageMeter()
        top1 = utils.AverageMeter()
        ...
        for i, (image, target) in enumerate(forget_loader):
            ...
            # compute output
            # output_clean = model(image)
            fms_u, output_clean = model(image, is_feature=1)
            loss = criterion(output_clean, target)
        ...
        for i, (image, target) in enumerate(retain_loader):
            ...
            # compute output
            # output_clean = model(image)
            fms_r, output_clean = model(image, is_feature=1)
            loss = criterion(output_clean, target)
